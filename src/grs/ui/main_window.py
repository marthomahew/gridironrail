    ...
    def create(self, action_handler: Callable[[ActionRequest], ActionResult], debug_gate: DebugGate):
        ...
        class MainWindow(QMainWindow):
            ...
            def _refresh_org(self) -> None:
                ...
                depth = data["depth_chart"]
                self.depth_table.setRowCount(len(depth))
                for i, row in enumerate(depth):
                    self.depth_table.setItem(i, 0, QTableWidgetItem(str(row["slot_role"])))
                    self.depth_table.setItem(i, 1, QTableWidgetItem(str(row["player_id"])))
                    self.depth_table.setItem(i, 2, QTableWidgetItem(str(row["priority"])))
